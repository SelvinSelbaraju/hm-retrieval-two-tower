...
class TFRecordWriter:
    ...
    def _parse_feature(self, feature_val: Union[str, float, int] , dtype: tf.dtypes.DType) -> tf.train.Feature:
        """
        ...
        if dtype == tf.string:
            ...
            raise TypeError(f"Invalid dtype {dtype} provided, must be one of {Feature.VALID_DTYPES}")
    ...
    def _get_features_from_row(self, row: tuple) -> bytes:
        ...
        """
        features = {
            feature.name: self._parse_feature(getattr(row, feature.name), feature.dtype)
            for feature in self.features
        }
        return tf.train.Example(features=tf.train.Features(feature=features)).SerializeToString()

    def write_tfrecords(self, df: pd.DataFrame, filepath: str, max_file_size: Optional[int] = None) -> None:
        """
        ...
        """
        logger.info(f"Removing .tfrecord suffix if present")
        ...
        if max_file_size:
            ...
            if len(df) % max_file_size:
                num_files += 1
            logger.info(f"Num files: {num_files} with max rows {max_file_size}")
        else:
            num_files = 1
            max_file_size = len(df)
            logger.info(f"Writing all rows to single file")
        for file_num in range(num_files):
            rows = df.iloc[file_num * max_file_size: (file_num+1) * max_file_size]

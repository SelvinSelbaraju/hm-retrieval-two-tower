...
class StaticIndex(AbstractKerasModel):
    ...
    """
    def __init__(self, k: int, input_features: List[Feature], candidates: tf.Tensor):
        super().__init__()
        self.k = k
    ...
    def get_input_signature(self) -> Dict[str, tf.TensorSpec]:
        return {f.name: tf.TensorSpec(shape=(None,1), dtype=f.dtype, name=f.name) for f in self.input_features}
